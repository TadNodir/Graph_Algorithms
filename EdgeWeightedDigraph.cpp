...
{
	std::vector<DirectedEdge> edgeResult;
	bool del = false;
	int start = e.from();
	for (DirectedEdge n : adj[start]) {
		if (n.to() != e.to()) {
			edgeResult.push_back(n);
		}
	}

	if (edgeResult.size() < adj[start].size()) {
		del = true;
	}
	adj[start].clear();
	adj[start] = edgeResult;
...
/**
	...
	*/
void EdgeWeightedDigraph::printGraph()
{
	std::vector<std::vector<DirectedEdge>> adjList = this->getAdj();
	int v = 0;
	int w = 0;
